...
>
> The [AgentCore CLI](https://github.com/aws/agentcore-cli), a separate tool from the AWS CLI above, does not appear to support `platformVersion` either as of September 19, 2026: searching that repository turns up no mention of it. It would normally be the tool for this workflow, which is why the scripts here call the API directly with boto3. Once the CLI supports it, use the CLI.
